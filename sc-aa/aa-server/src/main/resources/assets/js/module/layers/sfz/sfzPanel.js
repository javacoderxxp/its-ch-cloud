define(function(require) {
	var htmlStr = require('text!./sfzPanel.html');
	var Vue = require('vue');
	var map = require('mainMap');
	var vm = null;
	
	var show = function(sfzParam) {
				...
				save: function () {
					...
					$("#detailForm").validate({
						...
						submitHandler : function(){//验证通过的回调
							var url = "jtzt/sfz/save";
							$.ajax({
								type: "POST",
								url: url,
								data: JSON.stringify(vm.sfz),
								success: function(rslt){
									if(rslt.code === 200){
		...
		vueEureka.set("leftPanel", {
			vue: vm,
			description: "sfzPanel的vue实例"
		});
	}
	var loadJqGrid = function() {
		$("#jqGrid").jqGrid('GridUnload');
		var postDataTmp = {mc:vm.sfzQ.mc};
		$("#jqGrid").jqGrid({
			url: "jtzt/sfz/pageData",
			datatype: "json",
			...
			colModel: [
				...
				//{ label: '车位数', name: 'cws', width: 60, sortable:false/*, formatter: function(value, options, row){return value;}*/},
