define(function(require) {
	...
	markerOpts.icon = new IMAP.Icon("./assets/images/xfjg.png", new IMAP.Size(24, 24), new IMAP.Pixel(0, 0));

	var markers = [];//覆盖物列表
	var dataList = [];//数据列表
	//gblMapObjs.xfjgDataCluster;//聚合图
	var xfjgPanel = require('./xfjgPanel');
	...
	var applyDataToUI = function() {
		for(var i = 0; i < dataList.length; i++){
			...
			if(xfjg.zb){
				...
				marker.setTitle(xfjg.mc);
				markers.push(marker);
			}
		}
		gblMapObjs.xfjgMarkers = markers;
		//创建聚合管理对象
		map.plugin(['IMAP.DataCluster'], function(){
			gblMapObjs.xfjgDataCluster = new IMAP.DataCluster(map, markers, {
				maxZoom: 0, //比例尺级别
		...
		map.addEventListener(IMAP.Constants.ZOOM_END,function(){
	        ...
	        if(mZoom>=18){
	        	gblMapObjs.xfjgDataCluster.setMaxZoom(100,true);
	        }else{
	        	gblMapObjs.xfjgDataCluster.setMaxZoom(0,true);
	        }
	    });
		layer.msg('加载了'+markers.length+"个消防机构");
	};
	//清除数据
	var clear= function() {
		//清除界面
