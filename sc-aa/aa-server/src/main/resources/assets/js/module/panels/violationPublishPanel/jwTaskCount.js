define(function(require) {
	var htmlStr = require('text!./jwTaskCount.html');
	var Vue = require('vue');
	...
	var show = function() {
		...
		publishApp = new Vue({
			...
			methods: {
				query: function () {
					if(publishApp.publishQ.zdId || publishApp.publishQ.punisherNo){
						publishApp.isZd =true;
					}else{
						publishApp.isZd =false;
					}
					publishApp.publishQ.startTime = $("#startTime").val();
					publishApp.publishQ.endTime = $("#endTime").val();
					$.ajax({
						type: "GET",
					    url: "./qw/violationPunish/getJwTaskCount",
					    ...
					    success: function(rstl){
					    	if(rstl.code === 200){
					    		publishApp.punishList = rstl.punishMap.cList;
					    		publishApp.tCounts = rstl.punishMap.cTotal;
					    		if(publishApp.tCounts  == "" || publishApp.tCounts  == undefined){
