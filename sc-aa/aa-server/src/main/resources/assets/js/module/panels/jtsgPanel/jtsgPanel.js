	...
	var show = function(jtsgParam) {
		...
		vm = new Vue({
			...
			data: {
				...
				jtsgQ: {startDt: TUtils.formatDateTime000(new Date()), endDt: TUtils.formatDate(new Date())+" 23:59:59"}, //查询参数
				jtsg: jtsgParam,
				dataList : [],//数据列表
			...
			methods: {
				...
				markerClick : function(e) {
					...
					var msg = "案发地点："+data.eventAddr+"</br>时间："+data.eventDt
...
//						dataCluster.setMap(null);
	...
	var loadJqGrid = function() {
		...
		$("#jqGrid").jqGrid({
			url: "./jw/task/pageData",
			datatype: "json",
			postData: postDataTmp,
			colModel: [
				{ label: 'id', name: 'id', width: 5, key: true, hidden:true },
				{ label: '事故编号', name: 'eventId', width: 40, sortable:false/*, formatter: function(value, options, row){return value;}*/},
				{ label: '发生时间', name: 'eventDt', width: 40, sortable:false/*, formatter: function(value, options, row){return value;}*/},
				{ label: '发生地点', name: 'eventAddr', width: 60, sortable:false/*, formatter: function(value, options, row){return value;}*/},
				{ label: '办案单位', name: 'groupName', width: 40, sortable:false/*, formatter: function(value, options, row){return value;}*/},
				{ label: '报案人', name: 'eventReporter', width: 40, sortable:false/*, formatter: function(value, options, row){return value;}*/},
	...
	var loadVBadwList = function() {
	    $.get("aa/group/allData?zdFlag="+1, function(r){
			if(r.code == 200){
				vm.vBadwList = r.groupList;
				vm.vBadwList.unshift({groupName:'所有'});
