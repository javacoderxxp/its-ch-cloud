define(function(require) {
	...
	var markers = [];//覆盖物列表
	var dataList = [];//数据列表
	//gblMapObjs.clglDataCluster;//聚合图
	var clglPanel = require('./clglPanel');
	...
	var applyDataToUI = function() {
		for(var i = 0; i < dataList.length; i++){
			...
			if(clgl.zb){
				...
				marker.setTitle(clgl.id);
				markers.push(marker);
			}
		}
		gblMapObjs.clglMarkers = markers;
		//创建聚合管理对象
		map.plugin(['IMAP.DataCluster'], function(){
			gblMapObjs.clglDataCluster = new IMAP.DataCluster(map, markers, {
				maxZoom: 0, //比例尺级别
		...
		map.addEventListener(IMAP.Constants.ZOOM_END,function(){
			...
			if(mZoom>=18){
				gblMapObjs.clglDataCluster.setMaxZoom(100,true);
			}else{
				gblMapObjs.clglDataCluster.setMaxZoom(0,true);
			}
		});
		layer.msg('加载了'+dataList.length+"个车辆管理");
	};
	//清除数据
	var clear = function() {
		//清除界面
		...
		//清除聚合图
		if(gblMapObjs.clglDataCluster){
			gblMapObjs.clglDataCluster.clearMarkers();
//			dataCluster.setMap(null);
		}
		
		markers = [];
		gblMapObjs.clglMarkers = [];
		//清除数据
		dataList = [];
	};
	//单击事件
