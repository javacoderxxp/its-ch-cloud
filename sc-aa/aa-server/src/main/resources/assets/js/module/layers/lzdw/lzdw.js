define(function(require) {
	...
	markerOpts.icon = new IMAP.Icon("./assets/images/lzdw.png", new IMAP.Size(24, 24), new IMAP.Pixel(0, 0));

	var markers = [];//覆盖物列表
	var dataList = [];//数据列表
	//gblMapObjs.lzdwDataCluster;//聚合图
	var lzdwPanel = require('./lzdwPanel');
	...
	var applyDataToUI = function() {
		for(var i = 0; i < dataList.length; i++){
			...
			if(lzdw.zb){
				...
				marker.setTitle(lzdw.mc);
				markers.push(marker);
			}
		}
		gblMapObjs.lzdwMarkers = markers;
		//创建聚合管理对象
		map.plugin(['IMAP.DataCluster'], function(){
			gblMapObjs.lzdwDataCluster = new IMAP.DataCluster(map, markers, {
				maxZoom: 0, //比例尺级别
		...
		map.addEventListener(IMAP.Constants.ZOOM_END,function(){
	        ...
	        if(mZoom>=18){
	        	gblMapObjs.lzdwDataCluster.setMaxZoom(100,true);
	        }else{
	        	gblMapObjs.lzdwDataCluster.setMaxZoom(0,true);
	        }
	    });
		layer.msg('加载了'+markers.length+"个路政单位");
	};
	//清除数据
	var clear= function() {
		//清除界面
