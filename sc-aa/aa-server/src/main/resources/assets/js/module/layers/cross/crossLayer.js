define(function(require) {
	...
	var iconB = new IMAP.Icon("./assets/images/cross.png", new IMAP.Size(32, 32), new IMAP.Pixel(0, 0));
	markerOpts.icon= iconB;
	
	var markers = [];//覆盖物列表
	var dataList = [];//数据列表
	gblMapObjs.crossDataCluster;//聚合图
	var infoLabel = null;
	var crossPanel = require('./crossPanel');
	/*
	...
	*/
	...
	var applyDataToUI = function() {
		for(var i = 0; i < dataList.length; i++){
			...
			marker.setTitle(obj.mc+"("+obj.bh+")");
			markers.push(marker);
		}
		gblMapObjs.crossMarkers = markers;
		//创建聚合管理对象
		map.plugin(['IMAP.DataCluster'], function(){
			gblMapObjs.crossDataCluster = new IMAP.DataCluster(map, markers, {
				maxZoom: 0, //比例尺级别
		...
		map.addEventListener(IMAP.Constants.ZOOM_END,function(){
	        ...
	        if(mZoom>=18){
	        	gblMapObjs.crossDataCluster.setMaxZoom(100,true);
	        }else{
	        	gblMapObjs.crossDataCluster.setMaxZoom(0,true);
	        }
	    });
		layer.msg('加载了'+markers.length+"个路口");	
	};
	//清除数据
	var clear = function() {
		//清除界面元素
	...
	var markerClick = function(e) {
		...
		alert('路口编号:'+cross.bh+"<br/>路口名称:"+cross.mc);
	};
	//右击事件
	var rightClick = function(e) {
		var marker = e.target;
		var cross = e.target.data;
		//右键菜单
