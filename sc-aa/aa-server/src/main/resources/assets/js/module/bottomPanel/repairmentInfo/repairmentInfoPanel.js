define(function(require) {
	var htmlStr = require('text!./repairmentInfoPanel.html');
	var Vue = require('vue');
	...
	var loadData = function(parmData) {
		...
		// dataGrid初始化
		$("#dutyReleGrid").jqGrid({
			url:"qw/repairmentInfo/pageData?team="+parmData.team+"&aor="+parmData.aor+"&qwDt="+parmData.approveDt+"&opt="+parmData.status,
			datatype:"json",
			colModel:[
				{ label: '执勤时间', name:'repairDt', width:80, sortable:false},
				{ label: '勤务类型', name:'qwTypeDesc',width:50, sortable:false, formatter: function(value, options, row){
					return "隐患排查";}},
				...
				{ label: 'repairNum', name: 'repairNum', width: 60 , key:true, hidden:true },
	        ...
	        onSelectRow: function(rowid){
	        	...
	        	var picCnt;
	        	$.ajax({
	        	    url: "qw/repairmentInfo/getNumber?repairNum="+rowData.repairNum,
	        	    async: false,
	        	...
	        	for(var i = 0; i<slideCnt;i++){
	        		...
	        		for(var j = i*itemCnt; j<itemCnt*(i+1);j++){
	        			url = 'qw/repairmentInfo/getPicById?repairNum='+rowData.repairNum+'&seq='+(j+1)+"&random="+new Date().getTime();
	        ...
	        gridComplete:function(){
	        	...
	        	if(parmData.status == '2'){
	        		for(var i=0;i<ids.length;i++){
		        		...
		        		var rele= "<input class='btn btn-success btn-sm' style='font-size:10px;height:20px; line-height:8px;' type='button' value='确认关联' onclick=\"confirmRelevance('"+rowData.repairNum+'||'+parmData.id+'||'+parmData.qwType+"');\" />";
	        		...
	        		for(var i=0;i<ids.length;i++){
		        		var rowData = $("#dutyReleGrid").jqGrid('getRowData', ids[i]);
		        		var rele = "<input class='btn btn-success btn-sm' style='font-size:10px;height:20px; line-height:8px;' type='button' value='取消关联' onclick=\"cancelRelevance('"+rowData.repairNum+'||'+parmData.id+'||'+parmData.qwType+"');\" />";
...
var confirmRelevance = function(value){
	...
	$.ajax({
	    url: "qw/repairmentInfo/relevanceQw?repairNum="+parmArry[0]+"&id="+parmArry[1]+"&qwType="+parmArry[2]+"&qwRelevanceFlag="+"1",
	    success: function(rstl){
			if(rstl.code == 200){
				alert("关联成功！");
			}else{
...
var cancelRelevance = function(value){
	...
	var parmArry = value.split("||");
	$.ajax({
	    url: "qw/repairmentInfo/relevanceQw?repairNum="+parmArry[0]+"&id="+parmArry[1]+"&qwType="+parmArry[2]+"&qwRelevanceFlag="+"0",
	    success: function(rstl){
			if(rstl.code == 200){
				alert("取消关联成功！");
			}else{
