define(function(require) {
	var htmlStr = require('text!./schoolPanel.html');
	var Vue = require('vue');
	var map = require('mainMap');
	var vm = null;
	
	var show = function(schoolParam) {
		...
		vm = new Vue({
			...
			methods: {
				...
				edit: function () {
					...
					$.ajax({
						url: "jtzt/school/detail/"+id,
						success: function(rslt){
							if(rslt.code == 200){
								vm.title="编辑";
								vm.showList = false;
								vm.school = rslt.school;
							}else{
								alert(rslt.msg);
							}
						}
		...
		vueEureka.set("leftPanel", {
			vue: vm,
			description: "schoolPanel的vue实例"
		});
	}
	var loadJqGrid = function() {
		$("#jqGrid").jqGrid('GridUnload');
		var postDataTmp = {mc:vm.schoolQ.mc, lx:vm.schoolQ.lx};
		$("#jqGrid").jqGrid({
			url: "jtzt/school/pageData",
			datatype: "json",
			...
			colModel: [
				...
				{ label: '学校法人', name: 'xxfr', width: 60, sortable:false/*, formatter: function(value, options, row){return value;}*/},
				{ label: '法人电话', name: 'xxfrDh', width: 60, sortable:false/*, formatter: function(value, options, row){return value;}*/},
				{ label: '安全负责人', name: 'aqfzr', width: 60, sortable:false/*, formatter: function(value, options, row){return value;}*/},
			],
			viewrecords: true,
			height: 290,
			rowNum: 15,
